...
public class CommandHandler extends org.bukkit.command.Command {
	...
	public boolean execute(CommandSender sender, String label, String[] args) {		
		...
		while (!subcommand.subcommands.isEmpty()) {
			...
			for (Command subcommand2 : subcommand.subcommands) {
		...
		if (subcommand.callback == null) {
			...
			subcommand.helpHandler.sendSubcommandsMessage(subcommand, sender, label, args);
			return true;
		}
		...
		if (subcommand == this.command) {
			...
			String[] parameterStrings = ListUtils.subarray(args, 0, layer);
			List<Parameter<?>> parameters = subcommand.parameters;
			...
			for (int i = 0; i < parameters.size(); i++) {
				...
				if (parameter.isOptional() || (parameterStrings.length > (i + 1))) { // If this parameter was specified, or if the parameter is optional
					...
					String specifiedParameter = parameterStrings[i];
					try {
						parameter.setStringValue(specifiedParameter);
					} catch (ParameterParseException e) {
						subcommand.helpHandler.sendInvalidParameterMessage(subcommand, sender, label, args, parameter, e);
						return true;
					}
				} else {
					subcommand.helpHandler.sendMissingParameterMessage(subcommand, sender, label, args, parameter);
					return true;
				}
			}
